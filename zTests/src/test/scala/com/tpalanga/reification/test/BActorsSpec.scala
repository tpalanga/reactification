...
object BActorsSpec {
  ...
  val baseUrl = s"http://localhost:8082/events/tickets/$eventId"
}

class BActorsSpec extends AsyncFlatSpec with Matchers with EitherValues {

  import BActorsSpec._
  import ZApiProtocol._

  implicit val backend = AkkaHttpBackend()

  "ActorsService" should "process ticket requests and return download link" in {
